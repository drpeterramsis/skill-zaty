...
import { Search, Filter, BookOpen, Layers, Clock, XCircle, PlayCircle, Loader2, AlertCircle, Database, Copy, Check, RefreshCw, ArrowUp, PanelLeftClose, PanelLeftOpen, X, LogOut, Mail, ArrowRight, ShieldCheck, Lock } from 'lucide-react';
...
const SETUP_SQL = `-- 1. Create Courses Table (if not exists)
...
-- 2. Create Authorized Users Table (REQUIRED for Login)
...
-- 4. Create Access Policies (Public Read for Gatekeeper)
-- This allows the login page to check if an email exists without a password.
create policy "Public courses are viewable by everyone." on courses for select using ( true );
create policy "Public allowed_users check" on allowed_users for select using ( true );

-- 5. Insert Demo User (Optional - Replace with your email)
insert into allowed_users (email) values ('demo@skillzaty.com') on conflict do nothing;`;
...
const App: React.FC = () => {
  // Auth State (Local Storage Gate)
  const [currentUser, setCurrentUser] = useState<string | null>(() => {
    return typeof localStorage !== 'undefined' ? localStorage.getItem('skillzaty_user') : null;
  });
  
  // Data State
  const [courses, setCourses] = useState<Course[]>([]);
  const [loading, setLoading] = useState(false); // Changed to false initially until login
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // We do NOT send a magic link. We just check if the email exists in the whitelist table.
      ...
      if (error) {
        ...
        if (error.code === '42P01') {
             throw new Error("Configuration Error: 'allowed_users' table missing.");
        }
        throw error;
      }

      if (data) {
        // Success: Found in authorized list
        // Store in local storage to persist the "session"
        localStorage.setItem('skillzaty_user', data.email);
        setCurrentUser(data.email);
        setLoginMessage({ type: 'success', text: 'Access Granted.' });
      } else {
        // Fail: Not in list
      ...
      // Special check to help user setup
      if (err.message.includes('allowed_users') || err.message.includes('relation')) {
          setError('TABLE_NOT_FOUND'); // Trigger the SQL help screen
      }
    } finally {
  ...
  const handleLogout = () => {
    localStorage.removeItem('skillzaty_user');
    setCurrentUser(null);
    setCourses([]);
    setEmailInput('');
    setLoginMessage(null);
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 font-inter">
        {error === 'TABLE_NOT_FOUND' ? (
             /* SQL Setup Screen for Login Table */
             <div className="max-w-2xl w-full">
                <div className="bg-white p-6 rounded-lg shadow-lg border border-red-200">
                    <h2 className="text-xl font-bold text-slate-800 mb-4 flex items-center">
                        <Database className="mr-2 text-indigo-600" /> Database Setup Required
                    </h2>
                    <p className="text-slate-600 mb-4 text-sm">
                        To use the Authorized User Login system, you need to create the <code>allowed_users</code> table in Supabase.
                    </p>
                    <div className="relative mb-4">
                        <pre className="bg-slate-900 text-slate-300 p-4 rounded-lg text-xs overflow-x-auto font-mono leading-relaxed">
                            {SETUP_SQL}
                        </pre>
                        <button 
                            onClick={copySQL}
            ...
            <div className="flex flex-col items-center mb-8">
                <div className="bg-gradient-to-br from-indigo-600 to-violet-700 p-3 rounded-xl shadow-lg mb-4 transform hover:rotate-6 transition-transform">
            ...
            {loginMessage && (
                <div className={`mt-5 p-4 rounded-lg text-sm flex items-start gap-3 ${loginMessage.type === 'success' ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-red-50 text-red-700 border border-red-100'}`}>
                {loginMessage.type === 'success' ? <Check size={18} className="mt-0.5 shrink-0" /> : <Lock size={18} className="mt-0.5 shrink-0" />}
                <div>
                    <span className="font-semibold block mb-1">{loginMessage.type === 'success' ? 'Success' : 'Access Denied'}</span>
                    {loginMessage.text}
                </div>
                </div>
            )}
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center text-slate-500 font-inter">
        ...
        <h2 className="text-lg font-semibold text-slate-700">Loading Dashboard...</h2>
        <p className="text-sm text-slate-400 mt-2">Verifying credentials and fetching courses</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center text-slate-500 p-4 font-inter">
        ...
        <p className="text-sm max-w-md text-center mb-6 text-slate-600">
          {isTableError ? "Database tables are missing." : error}
        </p>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200 max-w-2xl w-full text-left mb-6">
            ...
            <div className="mt-4 flex items-start gap-2 text-xs text-amber-700 bg-amber-50 p-3 rounded border border-amber-100">
              ...
              <div>
                ...
                <p>1. Run the SQL above.</p>
                <p>2. Insert emails into <code>allowed_users</code> and course data into <code>courses</code>.</p>
